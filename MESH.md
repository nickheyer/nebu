...
Lockstep needs matching seats: same vendor, same runtime install version, same device count per node, because the ranks are symmetric. The mesh profile carries all three. A fabric link that the collective library takes as sockets instead of the RDMA device is the most common reason a cluster is slower than one node, so the seat's log line naming the transport is parsed and shown.
...
- lockstep: seats are the span's nodes whose device vendor, runtime install version, and device count match, on links of class fabric. Every seat holds every layer divided by the seat count. Seat counts that leave any seat over capacity are rejected with the shortfall.
...
`Shapes()` returns chain and lockstep when the install's version accepts `--nnodes`, relay when the NIXL connector imports in the install's environment, replicas always. Lockstep and chain candidates require every rank to have the same vendor, the same install version, and the same device count, because the ranks are symmetric. Speculative settings on the head are off in chain, which this runtime does not compose with pipeline parallel, and the planner's chain candidate says so when the request asked for them.
