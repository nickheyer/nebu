...
<PageHeader title={hostName() || 'Host'}>
  {#snippet meta()}
    {#if host}
      {#if live.settings?.hostLabel && live.settings.hostLabel !== host.hostname}<span class="font-mono text-fg-faint">{host.hostname}</span><span>·</span>{/if}
      <span class="font-mono">{host.os}/{host.arch}</span>
      <span>·</span>
      <span>probed {ago(host.probedAt, clock.now)}</span>
    {/if}
  {/snippet}
  <Button variant="outline" icon={RefreshCw} loading={probing} onclick={probe}>Probe</Button>
...
<div class="flex flex-col gap-6">
  ...
  {#if !host}
    <div class="panel"><Empty icon={Server} title="No profile yet" /></div>
  {:else}
    <section>
      <h2 class="mb-2 text-sm font-semibold text-fg">Memory</h2>
      <MemoryRows {host} cpu />
      {#if host.devices.some((d) => Object.keys(d.facts).length)}
        <div class="mt-3 grid grid-cols-1 gap-2 md:grid-cols-2 xl:grid-cols-3">
          {#each host.devices.filter((d) => Object.keys(d.facts).length) as d (d.id)}
            <details class="group rounded-lg border border-line bg-surface">
              <summary class="flex cursor-pointer items-center gap-2 px-3 py-2 text-xs text-fg-muted select-none">
                ...
                <span class="truncate text-fg">{d.name || d.id}</span>
                <span class="ml-auto shrink-0 text-fg-faint">{Object.keys(d.facts).length} facts</span>
              </summary>
    ...
    <section class="grid grid-cols-1 gap-4 xl:grid-cols-2">
      <Panel title="Memory pools" info="Where the planner places weights and cache" flush>
        <table class="tbl">
    ...
    <section class="grid grid-cols-1 gap-4 xl:grid-cols-2">
      <Panel title="Probes" info="Vendor tools and files read to build this profile" flush>
        <table class="tbl">
          ...
          <tbody>
            ...
            {:else}
              <tr><td colspan="3" class="text-xs text-fg-faint">No probes</td></tr>
            {/each}
          </tbody>
        </table>
      </Panel>

      <Panel title="Host facts" info="Values spec templates and constraints can reference">
        ...
        {/if}
        <p class="mt-3 text-[11px] text-fg-faint">Probed {when(host.probedAt)}</p>
      </Panel>
    </section>
  {/if}
</div>
