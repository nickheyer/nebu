<script lang="ts">
  ...
  let { id, label, active = '', dir = 'desc', num = false, onSort }: { id: string; label: string; active?: string; dir?: 'asc' | 'desc'; num?: boolean; onSort: (id: string) => void } = $props();
  const on = $derived(active === id);
</script>
