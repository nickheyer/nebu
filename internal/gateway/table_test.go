...
func TestTableLifecycle(t *testing.T) {
	...
	r := table.Pending("main", "s1", "repo:q4")
	if r.GetState() != v1.RouteState_ROUTE_STATE_PENDING || r.GetSlotId() != "s1" {
		t.Fatalf("pending %v", r)
	}
	if _, _, err := table.Acquire("main"); !errors.Is(err, ErrPending) {
		t.Fatalf("pending acquire %v", err)
	}
	if _, _, err := table.Acquire("nope"); !errors.Is(err, ErrNoRoute) {
		t.Fatalf("missing acquire %v", err)
	}
	r = table.Set("main", "i1", "s1", "http://a", "repo:q4", v1.ApiFlavor_API_FLAVOR_OPENAI)
	if r.GetState() != v1.RouteState_ROUTE_STATE_READY || r.GetInstanceId() != "i1" {
		t.Fatalf("ready %v", r)
	}
	table.Set("alias", "i1", "", "http://a", "repo:q4", v1.ApiFlavor_API_FLAVOR_OPENAI)
	ep, release, err := table.Acquire("main")
	...
	table.Drain("i1")
	if _, _, err := table.Acquire("main"); !errors.Is(err, ErrDraining) {
	...
	table.Set("keep", "i2", "s2", "http://b", "m", v1.ApiFlavor_API_FLAVOR_OPENAI)
...
func TestGatewayAuthAndStates(t *testing.T) {
	...
	table.Pending("starting", "s1", "m")
	table.Set("ready", "i1", "", upstream.URL, "m", v1.ApiFlavor_API_FLAVOR_OPENAI)
	table.Set("draining", "i2", "", upstream.URL, "m", v1.ApiFlavor_API_FLAVOR_OPENAI)
	table.Drain("i2")
	g := New(table, []string{"k1"}, slog.New(slog.NewTextHandler(io.Discard, nil)))
	g.SetListeners([]*v1.Listener{{Addr: "127.0.0.1:1", Shared: true}})
